/**
 * Message normalization — converts LLM messages to NormalizedBlock array.
 *
    ...
    if (msg.content && typeof msg.content !== 'string') {
      for (const part of msg.content) {
        if ((part as { type: string }).type === 'image') {
          blocks.push({ kind: 'user', text: `[image: ${(part as { mimeType: string }).mimeType}]`, sourceIndex: msgIndex });
        }
      }
    }
